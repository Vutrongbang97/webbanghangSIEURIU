...
const getProducts = async () => {
  const data = await axios({
    method: "GET",
    url: "https://6427f88946fd35eb7c48e546.mockapi.io/tech",
  });
...
const order = () => {
  ...
  Swal.fire({
    title: "<p class='title-alert success'>THÀNH TOÁN THÀNH CÔNG</p>",
    icon: "success",
    confirmButtonText: "OK",
  });
};
...
const filter = (brand) => {
  if(brand) {
    const productListFiltered = productList.filter((product) => product.brand === brand)
   return renderProductList(productListFiltered)
  }
...
const calculateBill = () => {
  ...
  document.getElementById("total").innerHTML = totalBill.toLocaleString('it-IT', {style : 'currency', currency : 'VND'});
};
// gọi hàm tính tổng bill khi vào website
calculateBill();
              ...
              <div class="d-flex justify-content-between">
                ...
                <div class="d-flex flex-row align-items-center">
                  <div style="margin-right: 30px;">
                    <span class="mb-0">${productPicked.salePrice.toLocaleString('it-IT', {style : 'currency', currency : 'VND'})}</span>
  ...
  const htmlString = products.reduce((accHTML, product) => {
    ...
    <div class="product card shadow-sm">
      ...
      <div class="card-body">
        ...
        <b class="text-danger">${product.salePrice.toLocaleString('it-IT', {style : 'currency', currency : 'VND'})} </b>
        <button style="background-color: black;" class="btn btn-primary w-100 text-start mt-4" onclick="addToCart(${product.id})" style>
        
        <img src="https://cdn-icons-png.flaticon.com/512/3163/3163175.png" style="width: 30px">
            <path d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2zm6.5 4.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3a.5.5 0 0 1 1 0z"/>
          </svg>
          <span  style="margin-left: 45px;">Order now</span>
        </button>
      </div>
    </div>
    `
